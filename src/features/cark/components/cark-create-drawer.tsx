...
import { X, Plus, Sparkles, Store, Settings, Gift, Copy, Wand2, Upload, Check, Code2, ArrowRight, Eye } from 'lucide-react'
...
// Entegrasyon adımları için Dialog bileşeni
function IntegrationSuccessDialog({
  open,
  onOpenChange,
  embedCode,
  storeId
  ...
  useEffect(() => {
    if (open) {
      ...
      const colors = ['#8B5CF6', '#EC4899', '#10B981', '#F59E0B', '#8B5CF6']
  ...
  const integrationSteps = [
    {
      title: '1. Embed Kodunu Kopyala',
      description: 'Aşağıdaki embed kodunu kopyalayın.',
      ...
      action: (
        ...
        >
          ...
          {copied ? 'Kopyalandı!' : 'Kodu Al'}
        </Button>
      ),
    },
    {
      title: '2. Store ID\'yi Kaydet',
      description: `Store ID'nizi not edin: ${storeId}. Bu ID backend entegrasyonu için gereklidir.`,
      ...
      action: (
        ...
        >
          {copiedStoreId ? <Check className="h-4 w-4 mr-1" /> : <Copy className="h-4 w-4 mr-1" />}
          {copiedStoreId ? 'Kopyalandı!' : 'ID\'yi Al'}
        </Button>
      ),
    },
    {
      title: '3. Backend\'i Deploy Edin',
      description: 'Backend API\'yi Railway/Render\'a deploy edin ve environment variables\'ı yapılandırın.',
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription>
                Çarkınız oluşturuldu ve kullanıma hazır. Entegrasyon için aşağıdaki adımları takip edin.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          {/* Store ID bilgisi */}
          <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 dark:from-emerald-950/30 dark:to-emerald-900/30 rounded-xl p-4 border border-emerald-200 dark:border-emerald-800">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-emerald-900 dark:text-emerald-100">Store ID</p>
                <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">{storeId}</p>
              </div>
              <Button
                size="sm"
                onClick={handleCopyStoreId}
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <pre className="text-xs bg-slate-950 text-slate-50 p-4 rounded-lg overflow-x-auto border border-slate-800">
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold">Entegrasyon Adımları</h3>
            <div className="space-y-3">
              {integrationSteps.map((step, index) => (
                <div
                  key={index}
                  className="flex items-start gap-4 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950"
                >
        ...
        <DialogFooter className="border-t border-slate-200 dark:border-slate-800 sm:justify-between gap-2">
          <Button
            variant="outline"
...
export function CarkCreateDrawer() {
  ...
  const { createWheel, wheelLimit, currentWheelCount } = useCark()
  const [logoPreview, setLogoPreview] = useState<string | null>(null)
  const [showSuccessDialog, setShowSuccessDialog] = useState(false)
  const [successData, setSuccessData] = useState<{ embedCode: string; storeId: string } | null>(null)

  const isLimitReached = currentWheelCount >= wheelLimit

  const [formData, setFormData] = useState<WheelFormData>(initialFormData)

  // Mağaza adından otomatik ID oluşturma fonksiyonu
  ...
  const handleSubmit = async () => {
    // Validasyon - storeId otomatik oluşturulur
    const autoStoreId = formData.storeId.trim() || generateStoreId(formData.storeName)
    if (!autoStoreId) {
    ...
    if (!formData.websiteUrl.trim() || formData.websiteUrl === 'https://www.') {
      alert('Lütfen Website URL girin')
      return
    }

    if (!formData.websiteUrl.startsWith('https://')) {
      alert('Website URL https:// ile başlamalıdır')
      return
    }
    ...
    // allowed_domains oluştur (websiteUrl'den)
    let allowedDomains: string[] = []
    try {
      const url = new URL(formData.websiteUrl)
      const hostname = url.hostname
      allowedDomains = [hostname]
      // www varsa www'sız versiyonunu da ekle
      if (hostname.startsWith('www.')) {
        allowedDomains.push(hostname.replace('www.', ''))
      } else {
        // www'sızsa www'li versiyonunu da ekle
        allowedDomains.push('www.' + hostname)
      }
    } catch {
      allowedDomains = [formData.websiteUrl.replace(/^https?:\/\//, '')]
    }
    ...
    if (result.success && result.wheel) {
      // Başarılı - drawer'ı kapat ve popup aç
      setOpen(false)
      setSuccessData({
        embedCode: result.wheel.embed_code || '',
        storeId: autoStoreId
      })
  ...
  return (
    <>
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetTrigger asChild>
          <Button
            ...
            title={isLimitReached ? `Maksimum ${wheelLimit} çark limitine ulaştınız` : ''}
          >
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
              ...
              <TabsContent value="store" className="space-y-5 sm:space-y-7 mt-3">
                <div className="space-y-4 sm:space-y-5">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setFormData({
                          ...formData,
                          storeName: newName,
                          storeId: generateStoreId(newName)
                        })
                      }}
                  ...
                  {/* Otomatik oluşturulan Mağaza ID */}
                  <div className="space-y-2">
                    <Label className="text-sm font-medium text-muted-foreground">
                      Mağaza ID <span className="text-xs">(Otomatik oluşturulur)</span>
                    </Label>
                    <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-muted/50 border border-border">
                      <span className="text-sm font-mono text-muted-foreground">
                        {formData.storeId || 'mağaza-adi-girin'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Çark sadece bu domain'de çalışacaktır. Lütfen https:// ile birlikte girin.
                    </p>
                  </div>
      ...
      <IntegrationSuccessDialog
        ...
        embedCode={successData?.embedCode || ''}
        storeId={successData?.storeId || ''}
      />
    </>
  )
}
