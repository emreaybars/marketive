...
export function CarkPrizeAnalytics() {
  ...
  // Debug
  React.useEffect(() => {
    console.log('CarkPrizeAnalytics render - wheels:', wheels?.length, 'wheelSpins:', wheelSpins?.length, 'loading:', loading)
  }, [wheels, wheelSpins, loading])
  ...
  const deviceData = React.useMemo(() => {
    return [
      { name: 'Mobil', value: 65, icon: Smartphone, color: '#3b82f6' },
      { name: 'Masaüstü', value: 30, icon: Monitor, color: '#60a5fa' },
      { name: 'Tablet', value: 5, icon: Tablet, color: '#93c5fd' },
    ]
  }, [])
            ...
            {deviceData.map((device) => (
              <div key={device.name} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">%{device.value}</span>
                </div>
