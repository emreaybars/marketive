...
export function CarkChart() {
  ...
  return (
    <Card className="overflow-hidden border border-slate-200 dark:border-slate-800 bg-white dark:bg-card">
      <CardHeader className="pb-4">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <div className="relative rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-950/30 dark:to-emerald-900/30 p-2 border border-emerald-200 dark:border-emerald-800">
                  <TrendingUp className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                </div>
              </div>
              <CardTitle className="text-base sm:text-lg text-slate-800 dark:text-slate-100">Çark Çevirme İstatistikleri</CardTitle>
            </div>
            <CardDescription className="flex items-center gap-2 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
            {/* Live indicator */}
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-500 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <span className="text-[10px] font-medium text-emerald-600 dark:text-emerald-400">CANLI</span>
            </div>

            {/* İstatistik Badge'leri */}
            <div className="hidden sm:flex items-center gap-2">
              <Badge variant="secondary" className="font-normal bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700">
                <span className="text-muted-foreground">Toplam:</span>{' '}
                <span className="font-semibold text-emerald-600 dark:text-emerald-400">{totalSpins.toLocaleString()}</span>
              </Badge>
              <Badge variant="secondary" className="font-normal bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700">
              ...
              {isCustomRange && (
                <>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        size="sm"
                        className="h-8 w-8 sm:w-auto sm:px-3 text-xs p-0 sm:p-auto border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800"
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-8 w-8 sm:w-auto sm:px-2 text-xs p-0 sm:p-auto hover:bg-slate-100 dark:hover:bg-slate-800"
                    onClick={() => {
      ...
      <CardContent className="space-y-4 px-4 pb-6">
        ...
        {filteredData.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2 text-slate-800 dark:text-slate-100">Henüz veri yok</h3>
          ...
          <>
            ...
            <div className="relative overflow-x-auto pb-2">
              ...
              <div className="absolute bottom-0 left-0 right-0 flex justify-end gap-1 px-2 pointer-events-none sm:hidden">
                ...
                <div className="h-1 w-2 rounded-full bg-emerald-500/40" />
              </div>
                  ...
                  <AreaChart data={filteredData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <XAxis
                      ...
                      tickFormatter={(value, index) => {
                        ...
                        return shouldShowTick(index) ? formattedDate : ''
                      }}
            ...
            {/* Alt bilgi - max değer */}
            <div className="flex items-center justify-center gap-2 text-xs text-slate-500 dark:text-slate-400 pt-4 border-t border-slate-100 dark:border-slate-800">
              <span>En yüksek:</span>
              <span className="font-semibold text-emerald-600 dark:text-emerald-400">{maxSpins.toLocaleString()}</span>
