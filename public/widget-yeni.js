...
(function () {
    ...
    function getConfig() {
        ...
        if (script) {
            API_URL = script.getAttribute('data-api-url');
            SHOP_TOKEN = script.getAttribute('data-shop-token');
        }
        return API_URL && SHOP_TOKEN;
    }
    ...
    function fetchWidgetData(token, callback) {
        if (!API_URL) {
            callback(new Error('API URL yapılandırılmamış'));
            return;
        }

        // Backend API'den widget verisini çek
        fetch(API_URL + '/api/widget?token=' + encodeURIComponent(token))
            .then(function (response) {
                if (!response.ok) {
                    throw new Error('Widget verisi alınamadı: ' + response.status);
                }
                return response.json();
            })
            .then(function (data) {
                if (!data || data.error) {
                    callback(new Error(data.error || 'Veri bulunamadı'));
                    return;
                }
    ...
    function checkContactUsed(contact, callback) {
        ...
        // Backend API üzerinden kontrol et
        fetch(API_URL + '/api/widget/check-contact', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                shopUuid: shopUuid,
                contact: contact,
                contactType: widgetData.shop.contactInfoType
            })
        })
            .then(function (response) {
                if (!response.ok) {
                    throw new Error('İletişim kontrolü başarısız: ' + response.status);
                }
                return response.json();
            })
            .then(function (data) {
                callback(null, data.used || false);
            })
            .catch(function (err) {
    ...
    function logSpin(prize, fullName, contact) {
        ...
        // Backend API üzerinden kaydet
        fetch(API_URL + '/api/widget/spin', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            ...
            .then(function (response) {
                if (!response.ok) {
                    throw new Error('Spin kaydı başarısız: ' + response.status);
                }
                return response.json();
            })
            .then(function (data) {
                log('Kayıt başarılı - Spin ID:', data.spin_id, 'Coupon:', data.coupon_code);

                // Prize nesnesine coupon kodunu ekle (modal gösterimi için)
                prize.coupon_code_result = data.coupon_code;
            })
            .catch(function (err) {
    ...
    function trackWidgetView() {
        ...
        // Backend API üzerinden takip et
        fetch(API_URL + '/api/widget/view', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    function buildWidgetHTML() {
        ...
        return '<div id="carkifelek-widget" style="' + getWidgetStyles() + '">' +
            '<button id="carkifelek-toggle" style="' + getToggleButtonStyles(widget.backgroundColor) + '">' +
            ...
            (widgetData.shop.logo_url ?
                '<img src="' + widgetData.shop.logo_url + '" alt="Logo" style="width: 100%; height: 100%; object-fit: contain; padding: 2px;" />' :
            ...
            '<div style="position: absolute; top: -8px; left: 20px; background: ' + widget.backgroundColor + '; padding: 0 8px; font-size: 10px; color: #888; text-transform: uppercase; letter-spacing: 1px;">Kupon Kodu</div>' +
    ...
    function getToggleButtonStyles(bgColor) {
