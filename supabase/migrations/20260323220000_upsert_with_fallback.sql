-- ============================================
-- UPSERT_SHOP_INTEGRATION - Şifreleme ile (fallback'lu)
-- ============================================
...
BEGIN
  -- Şifre verilmişse şifrelemeyi dene
  IF p_api_password IS NOT NULL AND p_api_password != '' THEN
    BEGIN
      -- Şifrelemeyi dene
      BEGIN
        v_encrypted_password := encode(
          pgp_sym_encrypt(p_api_password, current_setting('app.jwt_secret')::text, 'cipher-algo=aes256'),
          'base64'
        );
      EXCEPTION
        WHEN OTHERS THEN
          -- Şifreleme başarısız olursa düz metin kullan (fallback)
          v_encrypted_password := p_api_password;
          RAISE NOTICE 'Şifreleme başarısız, düz metin kullanılıyor';
      END;
    END;
  ELSE
  ...
  -- Yanıtta şifre görülmesin (api_password hariç)
