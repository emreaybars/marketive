-- ============================================
-- UPSERT_SHOP_INTEGRATION - Hem username hem password şifreleme
-- ============================================
...
DECLARE
  ...
  v_encrypted_username TEXT;
  v_encrypted_password TEXT;
BEGIN
  -- Username şifreleme
  IF p_api_username IS NOT NULL AND p_api_username != '' THEN
    v_encrypted_username := p_api_username; -- Şimdilik düz metin
  ELSE
    v_encrypted_username := NULL;
  END IF;
  ...
  IF v_integration.id IS NULL THEN
    ...
    VALUES (p_shop_id, p_platform_type, v_encrypted_username, v_encrypted_password, p_store_name)
    RETURNING * INTO v_integration;
  ELSE
    -- Mevcut entegrasyonu güncelle
    UPDATE shop_integrations
    SET
      api_username = COALESCE(v_encrypted_username, api_username),
  ...
  RETURN json_build_object(
    ...
    -- api_username ve api_password YANITTA GÖRÜNMEZ
  );
END;
